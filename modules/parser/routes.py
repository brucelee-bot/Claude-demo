...
对应高新技术产品（服务）：{rd_ps}"""
    ...
    template_instruction = ""
    if user_template:
        if field in {"purpose", "innovation", "result"}:
            template_without_placeholder_rule = (
                "3. 如果模板没有【】占位，必须严格沿用模板现有的编号、段落和句式结构，"
                "只补写当前栏目要求的内容，不得扩展为完整项目书，也不得增加其他栏目。"
            )
        else:
            template_without_placeholder_rule = (
                "3. 如果模板没有【】占位，请以模板为版式样例，替换企业名称、项目编号、年份、"
                "项目名称、项目时间等已知字段，并按该结构补写项目背景、研发内容及目标、计划进度和验收意见。"
            )
